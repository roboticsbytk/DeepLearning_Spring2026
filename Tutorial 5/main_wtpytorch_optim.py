...
class CNN(nn.Module):
    def __init__(self):
        ...
        # The 'pool' attribute is redefined here, which is not ideal, but doesn't cause the error directly.
        # self.pool = nn.MaxPool2d(2, 2)
        self.conv3 = nn.Conv2d(64, 64, 5)

        # Corrected input features for fc1 based on calculations: 64 channels * 1x1 spatial size = 64
    ...
    def forward(self, x):
        ...
        # Corrected flattening operation to preserve batch size and use correct feature count
        x = x.view(x.size(0), -1)
...
# Function to check accuracy (similar to the MNIST example)
